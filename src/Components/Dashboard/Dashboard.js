import { faAd, faEraser, faHome, faPersonBooth, faRegistered, faSignOutAlt, faTasks } from '@fortawesome/free-solid-svg-icons';
...
const Dashboard = () => {
    ...
    const loggedInUserStatus = async () => {
        await firestore.collection("users").get().then((querySnapshot) => {
            ...
            firestoreData.map(item => {
                ...
                const destructureData = { email: itemData.email, name: itemData.name, field: itemData.field, location: itemData.location, userStatus: itemData.userStatus, id: itemData.id }
    ...
    useEffect(() => {
        firestore.collection("admin").where("email", "==", loggedInUser.email)
            ...
            <div className="container-fluid">
                ...
                <div className="row">
                    ...
                    <main className="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                        ...
                        {selectedMenu === 'CONTRACTEE' && <ContracteeList userField={userField} userStatus={userStatus} />}
                        {selectedMenu === 'AVAILABLEJOBS' && <AvailableJobs userField={userField} />}
                    </main>
                </div>
            </div>
        </div>
